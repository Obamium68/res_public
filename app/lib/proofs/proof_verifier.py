...
def main():
    try:
        ...
        if len(sys.argv) != 2:
            print("Usage: python proof_verifier.py <proof_file_path>", file=sys.stderr)
            sys.exit(1)
        
        proof_file_path = sys.argv[1]
        print(f"Proof file path: {proof_file_path}", file=sys.stderr)
        
        if not os.path.exists(proof_file_path):
            print(f"Error: File does not exist: {proof_file_path}", file=sys.stderr)
            sys.exit(1)
        
        with open(proof_file_path, 'r') as f:
            proof_data = json.load(f)
        
        print(f"Proof data loaded successfully. Keys: {list(proof_data.keys())}", file=sys.stderr)
        ...
        error_result = {
            "valid": False,
            "message": f"Error in proof verification: {str(e)}",
            "errors": [f"Fatal error: {str(e)}"],
            "error_type": type(e).__name__
        }
        print(json.dumps(error_result, indent=2))
        print(f"Error in proof verification: {str(e)}", file=sys.stderr)
